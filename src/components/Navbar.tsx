  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-ink/15 bg-ivory/85 backdrop-blur">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-4">
        <Link href="/" className="text-xs font-semibold uppercase tracking-[0.4em] text-ink">
          {siteTitle}
        </Link>
        <nav className="hidden items-center gap-10 text-[0.7rem] font-medium uppercase tracking-[0.35em] text-ink/70 md:flex">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
            ...
            >
              <span className={pathname === item.href ? "text-ink" : "text-ink/60"}>
                {item.label}
              </span>
              <span
                className={`absolute -bottom-2 left-0 h-px w-full origin-left scale-x-0 bg-ink/70 transition group-hover:scale-x-100 ${
          ...
          <nav className="flex flex-col gap-6 text-xs font-medium uppercase tracking-[0.3em] text-ink/70">
            {navItems.map((item) => (
              <Link
                ...
                className={pathname === item.href ? "text-ink" : "text-ink/60"}
              >
                {item.label}
              </Link>
            ))}
